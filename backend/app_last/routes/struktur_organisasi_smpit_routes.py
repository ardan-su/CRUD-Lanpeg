...
from app.models import struktur_organisasi_smpit_model
from app.utils.response import success_response, error_response

bp = Blueprint("struktur_organisasi_smpit", __name__)

# GET semua data
@bp.route("/", methods=["GET"])
def get_all_struktur():
    try:
        data = struktur_organisasi_smpit_model.get_all()
        return success_response(data, "Get all struktur organisasi SMPIT success")
...
def get_struktur_by_id(id):
    try:
        data = struktur_organisasi_smpit_model.get_by_id(id)
        if not data:
...
def add_struktur():
    try:
        ...
        new_id = struktur_organisasi_smpit_model.create(data)
...
def edit_struktur(id):
    try:
        data = request.get_json()
        updated = struktur_organisasi_smpit_model.update(id, data)
        if updated:
...
def remove_struktur(id):
    try:
        deleted = struktur_organisasi_smpit_model.delete(id)
        if deleted:
