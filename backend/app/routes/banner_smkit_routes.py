...
from app.models import banner_smkit_model
from app.utils.response import success_response, error_response

bp = Blueprint("banner_smkit", __name__)

# GET all
@bp.route("/", methods=["GET"])
def get_banner():
    try:
        banners = banner_smkit_model.get_all_banner()
...
def get_banner_by_id(id):
    try:
        banner = banner_smkit_model.get_banner_by_id(id)
        if not banner:
...
def add_banner():
    try:
        ...
        new_id = banner_smkit_model.create_banner(data)
...
def edit_banner(id):
    try:
        data = request.get_json()
        updated = banner_smkit_model.update_banner(id, data)
        if updated:
...
def remove_banner(id):
    try:
        deleted = banner_smkit_model.delete_banner(id)
        if deleted:
