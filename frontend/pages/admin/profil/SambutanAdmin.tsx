...
import { Pencil, Trash2, Plus, Loader2, X, Upload } from 'lucide-react';
...
const FormModal: React.FC<FormModalProps> = ({ isOpen, onClose, formData, setFormData, handleSubmit, isEditing, status }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        const url = await uploadFile(file, 'sambutan');
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1 text-gray-700">Foto Ketua</label>
            ...
            {formData.foto_url && (
              <div className="mt-2 relative inline-block">
                <img src={formData.foto_url} alt="Preview" className="h-32 rounded-md border object-cover" />
                <button type="button" onClick={() => setFormData(prev => ({ ...prev, foto_url: '' }))} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 hover:bg-red-600">
                  <X size={12} />
                </button>
              </div>
            )}
        ...
        <footer className="flex justify-end gap-3 p-4 border-t bg-gray-50 rounded-b-xl">
          ...
          <button
            type="submit"
            form="modal-form" // This should match the form's id to trigger submission
...
const SambutanAdmin: React.FC = () => {
  ...
  return (
    <div className="relative">
      {notification && (
        <div
          className={`p-4 mb-4 rounded-md text-sm fixed top-24 right-8 z-[100] shadow-lg ${notification.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
            }`}
          role="alert"
        >
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Profil & Sambutan</h1>
          <p className="text-gray-500 mt-1">Kelola konten profil yayasan dan sambutan ketua.</p>
        </div>
      ...
      <div className="bg-white p-6 md:p-8 rounded-xl shadow-md">
        <h3 className="text-lg font-semibold mb-4 text-gray-800">Data Sambutan Saat Ini</h3>
        {status === 'loading' ? (
