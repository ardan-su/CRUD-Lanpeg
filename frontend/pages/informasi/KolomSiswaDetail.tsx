...
const KolomSiswaDetail: React.FC = () => {
    const { slug } = useParams<{ slug: string }>();
    const [article, setArticle] = useState<KolomSiswaAPI | null>(null);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    // Share functions
    const shareToFacebook = () => {
        const url = encodeURIComponent(getCanonicalShareUrl(`/informasi/kolom-siswa/${article?.slug || slug || ''}`));
        window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}`, '_blank', 'width=600,height=400');
    };

    const shareToTwitter = () => {
        const url = encodeURIComponent(getCanonicalShareUrl(`/informasi/kolom-siswa/${article?.slug || slug || ''}`));
        const text = encodeURIComponent(article?.judul || 'Karya Siswa');
        window.open(`https://twitter.com/intent/tweet?url=${url}&text=${text}`, '_blank', 'width=600,height=400');
    };

    const shareToWhatsApp = () => {
        const shareUrl = getCanonicalShareUrl(`/informasi/kolom-siswa/${article?.slug || slug || ''}`);
        const text = encodeURIComponent(`${article?.judul || 'Karya Siswa'} - ${shareUrl}`);
        ...
        const fetchDetail = async () => {
            ...
            try {
                ...
                if (result.success && Array.isArray(result.data)) {
                    const found = result.data.find((item: KolomSiswaAPI) => item.slug === slug);
                    if (found) setArticle(found);
                    else setError('Karya tidak ditemukan.');
                }
            } catch (err: any) {
                setError('Gagal memuat karya.');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    useEffect(() => {
        if (article) {
            updateMetaTags({
                ...
                url: getCanonicalShareUrl(`/informasi/kolom-siswa/${article.slug}`),
            });
        }
    }, [article]);
    ...
    return (
        <article className="max-w-4xl mx-auto">
            <Link to="/informasi/kolom-siswa" className="inline-flex items-center text-neutral-500 hover:text-primary mb-6 transition-colors font-medium">
                <ChevronLeftIcon className="h-5 w-5 mr-1" /> Kembali ke Kolom Siswa
            </Link>
            ...
            <div className="mb-8 text-center">
                ...
                <div className="flex flex-wrap justify-center items-center gap-4 md:gap-6 text-sm text-neutral-500 border-b border-neutral-200 pb-6">
                    <div className="flex items-center">
                        <UserIcon className="h-4 w-4 mr-2" /> {article.nama_siswa}
                    </div>
                    {article.kelas && (
                        <div className="flex items-center">
                    ...
                    {article.tahun_ajaran && (
                        <div className="flex items-center">
                            <svg className="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                            TA {article.tahun_ajaran}
                        </div>
                    )}
                    <div className="flex items-center">
                        <CalendarIcon className="h-4 w-4 mr-2" /> {formatDate(article.created_at)}
                    </div>
            ...
            <div className="mt-12 pt-8 border-t border-neutral-200">
                <h3 className="font-bold text-lg text-neutral-900 mb-4">Bagikan Karya Ini</h3>
...
export default KolomSiswaDetail;
