...
def create_app():
    ...
    # ========================
    # IMPORTANT: Use an absolute path so both file.save() and send_from_directory
    # resolve to the same physical directory.
    #
    # app.root_path  = .../backend/app/   (the package directory)
    # os.path.dirname(app.root_path) = .../backend/
    # So UPLOAD_FOLDER becomes .../backend/public/uploads  (absolute)
    #
    # Without this, file.save() writes to CWD/public/uploads (.../backend/public/uploads)
    # while send_from_directory looks in app.root_path/public/uploads
    # (.../backend/app/public/uploads) — a different path that doesn't exist → 404.
    UPLOAD_FOLDER_ABS = os.path.join(os.path.dirname(app.root_path), 'public', 'uploads')
    ...
    if not os.path.exists(UPLOAD_FOLDER_ABS):
        ...
        logger.info(f"Created upload folder: {UPLOAD_FOLDER_ABS}")
    else:
        logger.info(f"Upload folder ready: {UPLOAD_FOLDER_ABS}")
    ...
    from .routes import banner_dkv_routes
    app.register_blueprint(banner_dkv_routes.bp, url_prefix="/api/banner_dkv")
    ...
    from .routes import share_meta_routes
    app.register_blueprint(share_meta_routes.bp)
