...
import { Pencil, Trash2, Plus, Loader2, X, Layout, Target, Users, Trophy, Search, ChevronLeft, ChevronRight } from 'lucide-react';
import { uploadFile } from '../../../lib/uploadHelper';

const API_BASE = "http://localhost:5001/api";
const ENDPOINTS = {
    banner: `${API_BASE}/banner_smpit/`,
    visiMisi: `${API_BASE}/visi-misi-sasaran-smpit/`,
    struktur: `${API_BASE}/struktur-organisasi-smpit/`,
    prestasi: `${API_BASE}/prestasi-smpit/`,
};
...
const SMPITAdmin: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'banner' | 'visimisi' | 'struktur' | 'prestasi'>('banner');
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, fieldName: string) => {
        ...
        if (file) {
            try {
                const url = await uploadFile(file, 'smpit');
    ...
    const openModal = (item?: any) => {
        if (item) {
            setEditingId(item.id || item.no); // Prestasi uses 'no' sometimes
            setFormData({ ...item });
        } else {
            setEditingId(null);
            setFormData({});
            ...
            case 'struktur':
                return (
                    <>
                        ...
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Foto (Image URL/Upload)</label><input type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'image')} className="block w-full text-sm text-gray-500" /></div>
                    </>
                );
            case 'prestasi':
                return (
                    <>
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div><label className="block text-sm font-medium mb-1">Tahun</label><input type="text" name="tahun" value={formData.tahun || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                            <div><label className="block text-sm font-medium mb-1">Penyelenggara</label><input type="text" name="penyelenggara" value={formData.penyelenggara || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div><label className="block text-sm font-medium mb-1">Hasil (Juara)</label><input type="text" name="hasil_lomba" value={formData.hasil_lomba || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                            <div><label className="block text-sm font-medium mb-1">Lingkup</label><input type="text" name="lingkup" value={formData.lingkup || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        </div>
                    </>
                );
            default: return null;
    ...
    const renderTableContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'visimisi':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3 font-bold text-primary w-24">{item.tipe}</td>
                        <td className="p-3 text-sm line-clamp-2">{(item.deskripsi || '').substring(0, 100)}...</td>
                        <td className="p-3 text-right">
                            <button onClick={() => openModal(item)} className="text-yellow-600 p-2"><Pencil size={16} /></button>
                            <button onClick={() => handleDelete(item.id)} className="text-red-600 p-2"><Trash2 size={16} /></button>
                        </td>
            ...
            case 'prestasi':
                return currentItems.map(item => (
                    <tr key={item.no || item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3 text-sm">{item.tahun}</td>
                        <td className="p-3 font-medium">{item.jenis_lomba}</td>
                        <td className="p-3 font-bold text-green-600">{item.hasil_lomba}</td>
                        <td className="p-3 text-right">
                            <button onClick={() => openModal(item)} className="text-yellow-600 p-2"><Pencil size={16} /></button>
                            <button onClick={() => handleDelete(item.no || item.id)} className="text-red-600 p-2"><Trash2 size={16} /></button>
                        </td>
                    </tr>
                ));
        }
    ...
    return (
        <div>
            ...
            <div className="mb-8"><h1 className="text-3xl font-bold text-gray-800">Manajemen SMPIT</h1></div>
