...
import { Pencil, Trash2, Plus, Loader2, X, Layout, Target, BookOpen, Users, Trophy, Search, ChevronLeft, ChevronRight } from 'lucide-react';
import { uploadFile } from '../../../lib/uploadHelper';

const API_BASE = "http://localhost:5001/api";
const ENDPOINTS = {
    banner: `${API_BASE}/banner_smkit/`,
    visiMisi: `${API_BASE}/visi-misi-sasaran-smkit/`,
    kejuruan: `${API_BASE}/program-kejuruan-smkit/`,
    staf: `${API_BASE}/staf-smkit/`,
    prestasi: `${API_BASE}/prestasi-smkit/`,
};
...
const SMKITAdmin: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'banner' | 'visimisi' | 'kejuruan' | 'staf' | 'prestasi'>('banner');
    ...
    const getEndpoint = () => {
        switch (activeTab) {
            ...
            case 'kejuruan': return ENDPOINTS.kejuruan;
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, fieldName: string) => {
        ...
        if (file) {
            try {
                const url = await uploadFile(file, 'smkit');
    ...
    const renderFormContent = () => {
        switch (activeTab) {
            ...
            case 'visimisi':
                return (
                    <>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Kategori (Visi/Misi/Sasaran)</label><input type="text" name="kategori" value={formData.kategori || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
            ...
            case 'kejuruan':
                return (
                    <>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Nama Jurusan</label><input type="text" name="kategori" value={formData.kategori || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Deskripsi</label><textarea name="deskripsi" value={formData.deskripsi || ''} onChange={handleInputChange} rows={5} className="w-full border rounded px-3 py-2" /></div>
                    </>
                );
            case 'staf':
                ...
                return (
                    <>
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div><label className="block text-sm font-medium mb-1">Tahun Pelajaran</label><input type="text" name="tahun_pelajaran" value={formData.tahun_pelajaran || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                            <div><label className="block text-sm font-medium mb-1">Juara</label><input type="text" name="juara" value={formData.juara || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        </div>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Nama Lomba</label><input type="text" name="nama_kegiatan_lomba" value={formData.nama_kegiatan_lomba || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">No Urut</label><input type="number" name="no" value={formData.no || ''} onChange={handleInputChange} className="w-24 border rounded px-3 py-2" /></div>
                    </>
                );
            default: return null;
        }
    ...
    const renderTableContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'kejuruan':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3 font-bold text-primary w-32">{item.kategori}</td>
            ...
            case 'prestasi':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3 text-sm">{item.tahun_pelajaran}</td>
                        <td className="p-3 font-medium">{item.nama_kegiatan_lomba}</td>
                        <td className="p-3 font-bold text-green-600">{item.juara}</td>
                        <td className="p-3 text-right">
                            <button onClick={() => openModal(item)} className="text-yellow-600 p-2"><Pencil size={16} /></button>
                            <button onClick={() => handleDelete(item.id)} className="text-red-600 p-2"><Trash2 size={16} /></button>
                        </td>
    ...
    return (
        <div>
            ...
            <div className="mb-8"><h1 className="text-3xl font-bold text-gray-800">Manajemen SMKIT</h1></div>
            <div className="bg-white rounded-t-xl border-b flex overflow-x-auto">
                ...
                <TabButton active={activeTab === 'kejuruan'} onClick={() => setActiveTab('kejuruan')} icon={BookOpen} label="Kejuruan" />
