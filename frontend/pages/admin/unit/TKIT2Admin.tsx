...
import { Pencil, Trash2, Plus, Loader2, X, Layout, Target, BookOpen, Star, Users, Search, ChevronLeft, ChevronRight } from 'lucide-react';
import { uploadFile } from '../../../lib/uploadHelper';

const API_BASE = "http://localhost:5001/api";
const ENDPOINTS = {
    banner: `${API_BASE}/banner_tkit_2/`,
    visiMisi: `${API_BASE}/visi_misi_sasaran_tkit2/`,
    kegiatan: `${API_BASE}/kegiatan_pembelajaran_tkit2/`,
    program: `${API_BASE}/program_unggulan_tkit_2/`,
    struktur: `${API_BASE}/struktur_organisasi_tkit2/`,
};
...
const TKIT2Admin: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'banner' | 'visimisi' | 'kegiatan' | 'program' | 'struktur'>('banner');
    ...
    const getEndpoint = () => {
        switch (activeTab) {
            ...
            case 'kegiatan': return ENDPOINTS.kegiatan;
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, fieldName: string) => {
        ...
        if (file) {
            try {
                const url = await uploadFile(file, 'tkit2');
            ...
            case 'visimisi':
                return (
                    <>
                        ...
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Sasaran</label><textarea name="sasaran" value={formData.sasaran || ''} onChange={handleInputChange} rows={5} className="w-full border rounded px-3 py-2" /></div>
                    </>
                );
            case 'kegiatan':
                return (
                    <>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Judul</label><input type="text" name="judul" value={formData.judul || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Deskripsi</label><textarea name="deskripsi" value={formData.deskripsi || ''} onChange={handleInputChange} rows={3} className="w-full border rounded px-3 py-2" /></div>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Kelompok Belajar</label><textarea name="kelompok" value={formData.kelompok || ''} onChange={handleInputChange} rows={4} className="w-full border rounded px-3 py-2" placeholder="a. Kelompok Bermain..." /></div>
                    </>
                );
            case 'program':
                return (
                    <>
                        ...
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Deskripsi</label><textarea name="deskripsi" value={formData.deskripsi || ''} onChange={handleInputChange} rows={3} className="w-full border rounded px-3 py-2" /></div>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Gambar</label><input type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'image')} className="block w-full text-sm text-gray-500" /></div>
                    </>
                );
            case 'struktur':
                return (
        ...
        switch (activeTab) {
            ...
            case 'visimisi':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-3 text-sm line-clamp-2">{(item.sasaran || '').substring(0, 50)}...</td>
                        <td className="p-3 text-right">
                            <button onClick={() => openModal(item)} className="text-yellow-600 p-2"><Pencil size={16} /></button>
                            <button onClick={() => handleDelete(item.id)} className="text-red-600 p-2"><Trash2 size={16} /></button>
                        </td>
            ...
            case 'kegiatan':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3 font-medium">{item.judul}</td>
                        <td className="p-3 text-sm">{item.deskripsi}</td>
                        <td className="p-3 text-right">
                            <button onClick={() => openModal(item)} className="text-yellow-600 p-2"><Pencil size={16} /></button>
                            <button onClick={() => handleDelete(item.id)} className="text-red-600 p-2"><Trash2 size={16} /></button>
                        </td>
            ...
            case 'program':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3"><img src={item.image} className="h-10 w-10 object-contain" alt={item.nama_program} /></td>
    ...
    return (
        <div>
            ...
            <div className="mb-8"><h1 className="text-3xl font-bold text-gray-800">Manajemen TKIT 2</h1></div>
            <div className="bg-white rounded-t-xl border-b flex overflow-x-auto">
                ...
                <TabButton active={activeTab === 'kegiatan'} onClick={() => setActiveTab('kegiatan')} icon={BookOpen} label="Kegiatan" />
