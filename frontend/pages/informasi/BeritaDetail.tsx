...
const BeritaDetail: React.FC = () => {
    ...
    const [article, setArticle] = useState<BeritaArtikelAPI | null>(null);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    // Share functions
    const shareToFacebook = () => {
        const url = encodeURIComponent(getCanonicalShareUrl(`/informasi/berita/${article?.slug || slug || ''}`));
        window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}`, '_blank', 'width=600,height=400');
    };

    const shareToTwitter = () => {
        const url = encodeURIComponent(getCanonicalShareUrl(`/informasi/berita/${article?.slug || slug || ''}`));
        const text = encodeURIComponent(article?.judul || 'Artikel Menarik');
        window.open(`https://twitter.com/intent/tweet?url=${url}&text=${text}`, '_blank', 'width=600,height=400');
    };

    const shareToWhatsApp = () => {
        const shareUrl = getCanonicalShareUrl(`/informasi/berita/${article?.slug || slug || ''}`);
        const text = encodeURIComponent(`${article?.judul || 'Artikel Menarik'} - ${shareUrl}`);
        ...
        const fetchArticle = async () => {
            ...
            try {
                ...
                if (result.success && Array.isArray(result.data)) {
                    const foundArticle = result.data.find((item: BeritaArtikelAPI) => item.slug === slug);
                    if (foundArticle) {
    ...
    useEffect(() => {
        if (article) {
            updateMetaTags({
                title: article.judul,
                description: stripHtml(article.deskripsi || '').substring(0, 200),
                image: article.gambar,
                url: getCanonicalShareUrl(`/informasi/berita/${article.slug}`),
            });
        }
    }, [article]);
    ...
    if (isLoading) {
        return (
            <div className="animate-pulse max-w-4xl mx-auto py-12 px-4">
                <div className="h-8 bg-neutral-200 rounded w-1/4 mb-6"></div>
                <div className="h-64 bg-neutral-200 rounded-xl mb-8 w-full"></div>
    ...
    if (error || !article) {
        return (
            <div className="text-center py-20 px-4">
                ...
                <p className="text-neutral-600 mb-6">{error || 'Artikel yang Anda cari tidak ditemukan.'}</p>
                <Link to="/informasi/berita" className="text-primary font-semibold hover:underline">
    ...
    // Helper to robustly clean and decode HTML content from API
    const cleanAndDecodeHtml = (content: string) => {
        if (!content) return '';

        let cleaned = content;

        // 1. Handle JSON-stringified artifacts first (e.g. \r\n inside the string)
        // Global replace for literal escaped characters
        ...
        // 2. Decode HTML Entities (recursive to handle double encoding like &lt;p&gt;)
        ...
        // Attempt decode up to 3 times to ensure all layers are peeled (e.g., &amp;lt;p&amp;gt;)
        for (let i = 0; i < 3; i++) {
    ...
    if (article.content_delta) {
        ...
        // Fallback: If after cleaning it still looks like plain text (no tags), add basic formatting
    ...
    return (
        <article className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <Link to="/informasi/berita" className="inline-flex items-center text-neutral-500 hover:text-primary mb-6 transition-colors font-medium">
