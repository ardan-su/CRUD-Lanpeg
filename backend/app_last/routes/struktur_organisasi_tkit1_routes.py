...
from app.models import struktur_organisasi_tkit1_model
from app.utils.response import success_response, error_response

bp = Blueprint("struktur_organisasi_tkit1", __name__)

# GET semua data
@bp.route("/", methods=["GET"])
def get_all_struktur():
    try:
        data = struktur_organisasi_tkit1_model.get_all_struktur()
        return success_response(data, "Get all struktur organisasi success")
...
def get_struktur_by_id(id):
    try:
        data = struktur_organisasi_tkit1_model.get_struktur_by_id(id)
        if not data:
...
def add_struktur():
    try:
        ...
        new_id = struktur_organisasi_tkit1_model.create_struktur(data)
...
def edit_struktur(id):
    try:
        data = request.get_json()
        updated = struktur_organisasi_tkit1_model.update_struktur(id, data)
        if updated:
...
def remove_struktur(id):
    try:
        deleted = struktur_organisasi_tkit1_model.delete_struktur(id)
        if deleted:
