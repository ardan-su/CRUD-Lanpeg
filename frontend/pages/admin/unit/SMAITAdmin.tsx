...
const ENDPOINTS = {
    banner: `${API_BASE}/banner_smait/`,
    visiMisi: `${API_BASE}/visi-misi-sasaran-smait/`,
    kurikulum: `${API_BASE}/kurikulum-smait/`,
    program: `${API_BASE}/program-unggulan-smait/`,
    staf: `${API_BASE}/staf-smait/`,
    prestasi: `${API_BASE}/prestasi-smait/`,
};
...
const SMAITAdmin: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'banner' | 'visimisi' | 'kurikulum' | 'program' | 'staf' | 'prestasi'>('banner');
    ...
    const getEndpoint = () => {
        switch (activeTab) {
            ...
            case 'staf': return ENDPOINTS.staf;
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, fieldName: string) => {
        ...
        if (file) {
            try {
                const url = await uploadFile(file, 'smait');
    ...
    const renderFormContent = () => {
        switch (activeTab) {
            ...
            case 'program':
                return (
                    <>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Nama Program</label><input type="text" name="nama_program" value={formData.nama_program || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Gambar/Icon</label><input type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'image')} className="block w-full text-sm text-gray-500" /></div>
                    </>
                );
            case 'staf':
                return (
                    <>
                        ...
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Foto</label><input type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'image')} className="block w-full text-sm text-gray-500" /></div>
                    </>
                );
            case 'prestasi':
                return (
                    <>
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div><label className="block text-sm font-medium mb-1">Tanggal</label><input type="date" name="tgl" value={formData.tgl || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                            <div><label className="block text-sm font-medium mb-1">Nama Siswa</label><input type="text" name="nama" value={formData.nama || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        </div>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Event</label><input type="text" name="event" value={formData.event || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div><label className="block text-sm font-medium mb-1">Peringkat</label><input type="text" name="peringkat" value={formData.peringkat || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                            <div><label className="block text-sm font-medium mb-1">Level</label><input type="text" name="level" value={formData.level || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                        </div>
                        <div className="mb-4"><label className="block text-sm font-medium mb-1">Penyelenggara</label><input type="text" name="penyelenggara" value={formData.penyelenggara || ''} onChange={handleInputChange} className="w-full border rounded px-3 py-2" /></div>
                    </>
                );
            default: return null;
        }
    ...
    const renderTableContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'kurikulum':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3 font-bold text-primary w-32">{item.tipe || item.kategori}</td>
                        <td className="p-3 text-sm line-clamp-2">{(item.deskripsi || '').substring(0, 80)}...</td>
                        <td className="p-3 text-right">
                            <button onClick={() => openModal(item)} className="text-yellow-600 p-2"><Pencil size={16} /></button>
                            <button onClick={() => handleDelete(item.id)} className="text-red-600 p-2"><Trash2 size={16} /></button>
                        </td>
            ...
            case 'program':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3"><img src={item.image} className="h-10 w-10 object-contain" alt="Icon" /></td>
                        <td className="p-3 font-medium">{item.nama_program}</td>
                        <td className="p-3 text-right">
                            <button onClick={() => openModal(item)} className="text-yellow-600 p-2"><Pencil size={16} /></button>
                            <button onClick={() => handleDelete(item.id)} className="text-red-600 p-2"><Trash2 size={16} /></button>
                        </td>
            ...
            case 'staf':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3"><img src={item.image} className="h-10 w-10 rounded-full object-cover" alt={item.nama} /></td>
            ...
            case 'prestasi':
                return currentItems.map(item => (
                    <tr key={item.id} className="border-b hover:bg-gray-50">
                        <td className="p-3 text-sm">{item.tgl}</td>
                        <td className="p-3 font-medium">{item.nama}</td>
                        <td className="p-3 text-sm">{item.event}</td>
                        <td className="p-3 font-bold text-green-600">{item.peringkat}</td>
                        <td className="p-3 text-right">
                            <button onClick={() => openModal(item)} className="text-yellow-600 p-2"><Pencil size={16} /></button>
                            <button onClick={() => handleDelete(item.id)} className="text-red-600 p-2"><Trash2 size={16} /></button>
                        </td>
    ...
    return (
        <div>
            ...
            <div className="mb-8"><h1 className="text-3xl font-bold text-gray-800">Manajemen SMAIT</h1></div>
            <div className="bg-white rounded-t-xl border-b flex overflow-x-auto">
                ...
                <TabButton active={activeTab === 'staf'} onClick={() => setActiveTab('staf')} icon={Users} label="Staf" />
